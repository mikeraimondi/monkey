...
func TestDefine(t *testing.T) {
	...
	if c != expected["c"] {
		t.Errorf("wrong value for c. expected %+v, got %+v", expected["c"], c)
	}
	d := firstLocal.Define("d")
	if d != expected["d"] {
		t.Errorf("wrong value for d. expected %+v, got %+v", expected["d"], d)
	}
	...
	if e != expected["e"] {
		t.Errorf("wrong value for e. expected %+v, got %+v", expected["e"], e)
	}
	f := secondLocal.Define("f")
	if f != expected["f"] {
		t.Errorf("wrong value for f. expected %+v, got %+v", expected["f"], f)
	}
}
...
func TestResolveFree(t *testing.T) {
	...
	for _, tt := range tests {
		...
		if len(tt.table.FreeSymbols) != len(tt.expectedFreeSymbols) {
			t.Errorf("wrong number of free symbols. got %d, expected %d",
		...
		for i, sym := range tt.expectedFreeSymbols {
			...
			if result != sym {
				t.Errorf("wrong free symbol. got %+v, expected %+v",
					result, sym)
			}
		}
	}
